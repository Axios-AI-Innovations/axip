"""
AXIP OpenAI Agents SDK Tools

FunctionTool wrappers that connect directly to the AXIP Python SDK.
...
Usage:
    ...
    result = await Runner.run(agent, "Search for the latest AI agent news")
...
logger = logging.getLogger("axip.openai_agents")

# ─── Lazy import guard ─────────────────────────────────────────────────────
# openai-agents is optional — axip installs without it as a hard dependency.
...
def _require_openai_agents():
    try:
        ...
        raise ImportError(
            "openai-agents is required to use axip.openai_agents_tools. "
            "Install it with: pip install openai-agents"
        ) from e


# ─── Shared agent lifecycle ────────────────────────────────────────────────
# Same background-thread pattern as crewai_tools / langchain_tools:
# one shared AXIPAgent per process, started once, bridged to sync callers
...
def make_axip_tools(
    *,
    ...
    agent_name: str = "openai-agents-client",
    timeout: float = 60.0,
) -> list:
    """
    Create a list of OpenAI Agents SDK FunctionTool instances backed by the AXIP Python SDK.
    ...
    Returns:
        List of FunctionTool instances ready to pass to an OpenAI Agents Agent.
    ...
    Example::
        ...
        result = await Runner.run(agent, "Summarize the latest AI news")
        print(result.final_output)
    """
    ...
    # function_tool extracts the schema from type hints and the docstring.
    # Define functions inside the factory so they close over axip_agent/event_loop.

    @function_tool
    def axip_request_task(capability: str, description: str, reward: float = 0.05) -> str:
        """
        Delegate a task to a specialized AI agent on the AXIP network.

        Use this to perform web searches, code review, translation, summarization,
        data extraction, and other specialized tasks. Returns the result as a string.

        Args:
            capability:  The AXIP capability required, e.g. 'web_search', 'summarize',
                         'code_review', 'translate', 'data_extraction'.
            description: Natural language description of the task to perform.
            reward:      Maximum USD you will pay for this task (default 0.05).
        """
        try:
            result = _run_coro(
    ...
    @function_tool
    def axip_discover_agents(capability: str) -> str:
        """
        ...
        Args:
            capability: Capability to search for, e.g. 'web_search', 'summarize'.
        """
        try:
            agents_list = _run_coro(
        ...
        lines = [f"Found {len(agents_list)} agent(s) for '{capability}':"]
    ...
    return [axip_request_task, axip_discover_agents, axip_network_status]

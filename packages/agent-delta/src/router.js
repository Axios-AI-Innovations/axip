/**
 * Agent Delta (Sentinel) — Simplified Router
 *
 * Single-tier passthrough to local Ollama.
 * Only uses qwen3:1.7b. No Anthropic, no budget gating, no tier system.
 * Mirrors Eli's route() signature for consistency.
 */
...
/**
 * Route a task to the local Ollama model (qwen3:1.7b).
 *
 * @param {Object} params
 * @param {string} params.prompt - The user prompt
 * @param {string} [params.system] - System prompt
 * @param {string} params.taskName - For cost tracking
 * @param {number} [params.maxTokens=512] - Max output tokens
 * @param {number} [params.temperature=0.2] - Sampling temperature (low for sentinel precision)
 * @returns {Promise<{ text: string, model: string, provider: string, inputTokens: number, outputTokens: number, durationMs: number }>}
 */
export async function complete({ prompt, system = '', taskName, maxTokens = 512, temperature = 0.2 }) {
